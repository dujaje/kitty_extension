      ...
      entry["messaging"].each do |messaging|
        if messaging["read"]
          ...
          unless text.nil?
            ...
            elsif text.downcase.include?("question 1") || text.downcase.include?("question one")
              my_reply = how_does_kitty_work_reply(sender)
            elsif text.downcase.include?("question 2") || text.downcase.include?("question two")
              my_reply = does_kitty_intrude_reply(sender)
            elsif text.downcase.include?("question 3") || text.downcase.include?("question three")
